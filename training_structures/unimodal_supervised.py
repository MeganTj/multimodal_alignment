...
def train(encoder, head, train_dataloader, valid_dataloader, total_epochs, early_stop=False, optimtype=torch.optim.AdamW, lr=0.001, weight_decay=0.0, criterion=nn.CrossEntropyLoss(), auprc=False, save_encoder='encoder.pt', save_head='head.pt', 
         is_packed=False, modalnum=0, recon_modalnum=None, task='classification', track_complexity=True):
    ...
    def _trainprocess():
        ...
        if task == "reconstruction":
            assert recon_modalnum is not None
        for epoch in range(total_epochs):
            ...
            for j in train_dataloader:
                op.zero_grad()
                if isinstance(j[modalnum], tuple) or isinstance(j[modalnum], list):
                    out = model((j[modalnum][0].float().to(device), j[modalnum][1].to(device)))
                else:
                    out = model(j[modalnum].float().to(device))
                
                if task == "reconstruction":
                    loss = criterion(out, j[recon_modalnum].float().to(device))
                else:
            ...
            if task == "classification":
                ...
                if f1_macro > bestf1:
                    ...
                    patience += 1
            elif task == "regression" or task == "reconstruction":
                print("Epoch "+str(epoch)+" valid loss: "+str(valloss))
...
def single_test(encoder, head, test_dataloader, auprc=False, modalnum=0, recon_modalnum=None, task='classification', criterion=None, save_preds=None):
    ...
    with torch.no_grad():
        ...
        elif "posneg-classification" in task:
            trueposneg = true
            accs = eval_affect(trueposneg, pred)
            acc2 = eval_affect(trueposneg, pred, exclude_zero=False)
            print("acc: "+str(accs) + ', ' + str(acc2))
            if "mean" in task:
                mean_acc = (accs + acc2) / 2
                print(f"Mean: {mean_acc}")
                return mean_acc
            else:
                print(f"Acc over all data: {acc2}")
                return acc2
        else:
            return {'MSE': (totalloss / totals).item()}


def test(encoder, head, test_dataloaders_all, dataset='default', method_name='My method', auprc=False, modalnum=0, recon_modalnum=None, task='classification', criterion=None, no_robust=False, save_preds=None):
    ...
    if no_robust:
        return single_test(encoder, head, test_dataloaders_all, auprc, modalnum, recon_modalnum, task, criterion, save_preds=save_preds)
