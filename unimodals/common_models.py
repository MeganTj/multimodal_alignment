...
class CustomMLP(torch.nn.Module):
    ...
    def __init__(self, indim, hiddim, num_hidden=1, nonlin="relu", bottleneck_dim=None):
        """Initialize two-layered perceptron.
        ...
        self.bottleneck_idx = -1
        layer_outdim = hiddim
        if bottleneck_dim is not None:
            self.bottleneck_idx = num_hidden // 2 - 1
            if self.bottleneck_idx <= 0:
                layer_outdim = bottleneck_dim
        layers_dict = OrderedDict({"fc1": nn.Linear(indim, layer_outdim)})
        for layer in range(0, num_hidden):
            if nonlin == "relu":
                nonlin_layer = nn.ReLU()
            elif nonlin == "leaky_relu":
                nonlin_layer = nn.LeakyReLU(0.1)
            elif nonlin == "tanh":
                nonlin_layer = nn.Tanh()
            else:
                raise NotImplementedError
            layers_dict[f"{nonlin}{layer+1}"] = nonlin_layer
            if layer < num_hidden - 1:
                layer_indim = hiddim
                layer_outdim = hiddim
                if bottleneck_dim is not None:
                    if layer == self.bottleneck_idx - 1:
                        layer_outdim = bottleneck_dim
                    elif layer == self.bottleneck_idx:
                        layer_indim = bottleneck_dim
                layers_dict[f"fc{layer+2}"] = nn.Linear(layer_indim, layer_outdim)
...
class Transformer(nn.Module):
    ...
    def __init__(self, n_features, max_seq_length, embed_dim, n_layers, nhead=1, norm_first=False, use_cls_token=False, 
                 tokenize=False, final_hidden_dim=None):
        ...
        self.dim_feedforward = self.embed_dim
        self.n_layers = n_layers
        self.tokenize = tokenize
        self.bottleneck_idx = None
        self.proj = nn.Linear(n_features, embed_dim)
        layer = nn.TransformerEncoderLayer(d_model=self.embed_dim, nhead=nhead, dim_feedforward=embed_dim, norm_first=norm_first )
        self.transformer = nn.TransformerEncoder(layer, num_layers=self.n_layers)
        # Parameters/Embeddings
        self.use_cls_token = use_cls_token
        self.last_proj = None
        if final_hidden_dim is not None:
            self.last_proj = nn.Linear(self.embed_dim, final_hidden_dim)
        if self.use_cls_token:
            self.cls_token = nn.Parameter(torch.randn(1, 1, self.embed_dim))
        self.pos_embedding = nn.Parameter(torch.randn(1, self.use_cls_token  + max_seq_length, embed_dim))
    ...
    def forward(self, x):
        ...
        if isinstance(x, tuple):
            x, src_key_padding_mask = x
            if src_key_padding_mask is not None and self.use_cls_token:
                cls_token_pad = torch.tensor([[False]]).repeat(x.shape[0], 1).to(x.device)
                src_key_padding_mask = torch.cat([cls_token_pad, src_key_padding_mask], dim=1)
        if self.tokenize:
        ...
        x = x + self.pos_embedding[:, : T + self.use_cls_token]
        # Result is (seq, batch, embed_dim)
        x = x.permute([1, 0, 2])
        if self.use_cls_token:
            x = self.transformer(x, src_key_padding_mask=src_key_padding_mask)[0, :, :]
        else:
            # Mean over sequence
            x = self.transformer(x, src_key_padding_mask=src_key_padding_mask).mean(dim=0)
        if hasattr(self, "last_proj") and self.last_proj is not None:
            x = self.last_proj(x)
        return x
...
class VisionTransformer(Transformer):
    ...
    def __init__(self, n_features, max_seq_length, embed_dim, n_layers, nhead=1, norm_first=False,
                use_cls_token=False, patch_size=None, tokenize=False, final_hidden_dim=None):
        """Initialize Transformer object.
        ...
        super(VisionTransformer, self).__init__(n_features, max_seq_length, embed_dim, n_layers, nhead=nhead, norm_first=norm_first, use_cls_token=use_cls_token, tokenize=tokenize, final_hidden_dim=final_hidden_dim)
    ...
    def forward(self, x):
        ...
        if isinstance(x, tuple) or isinstance(x, list):
            x, src_key_padding_mask = x
        if len(x.shape) == 5:
            # Assume x is B, C, H', W', p_H * p_W
            assert x.shape[1] == 1
            x = x.flatten(1, 3)
        elif self.patch_size:
            x = self.img_to_patch(x, self.patch_size)
        return super().forward((x, src_key_padding_mask))